...
def get_levels_chart(symbol):

    df_candles = pd.DataFrame(get_candles_by_symbol(symbol), columns=['id','Symbol','Open','High','Low','Close','Date'])
    ...
    levels_list = get_levels_by_symbol(symbol)
    ...
    for level in levels_list:
        ...
        fig.add_shape(type="line", x0=level[4], y0=level[2],
                       x1=df_candles['Date'].iloc[-1], y1=level[2], line=dict(color=color, width=3))
...
def get_chart_with_impulse(df, impulse,level_list, tf):
    ...
    if len(level_list) != 0:
        for level in level_list:
            ...
            #                           x0=level[5], y0=level[3], x1=df['Date'].iloc[-1], y1=level[3],
